...
/*   Updated: 2025/09/25 13:22:02 by mtangalv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void print_branch(int depth, int is_last, int is_right)
{
	...
	if (depth > 0)
	{
		...
		if (is_right)
			ft_printf("\033[1;36m[R]\033[0m ");
		else
			ft_printf("\033[1;35m[L]\033[0m ");
	}
}
...
static void print_args(char **args)
{
	...
	if (!args || !args[0])
		return;
	ft_printf("\033[3mArgs:\033[0m ");
	while (args[i])
	{
		ft_printf("\033[1;33m\"%s\"\033[0m", args[i]);
		if (args[i + 1])
			ft_printf(", ");
		i++;
	}
...
static void print_ast_recursive(t_ast *node, int depth, int is_last, int is_right)
{
	...
	ft_printf("\033[1m%s\033[0m", token_type_to_str(node->value.type));
	if (node->value.cmd && *node->value.cmd)
		ft_printf(": \033[3;32m\"%s\"\033[0m", node->value.cmd);
	if (node->value.args)
	{
		ft_printf(" ");
...
{
	ft_printf("\n\033[1;36m╔════════════════════════════════════════════════╗\033[0m\n");
	ft_printf("\033[1;36m║          ABSTRACT SYNTAX TREE (AST)             ║\033[0m\n");
...
{
	ft_printf("\n\033[1;36m╔════════════════════════════════════════════════╗\033[0m\n");
	ft_printf("\033[1;36m║                  END OF AST                     ║\033[0m\n");
	ft_printf("\033[1;36m╚════════════════════════════════════════════════╝\033[0m\n\n");
}
...
void debug_print_ast(t_ast *root)
{
	...
	if (!root)
		ft_printf("\033[1;31m[EMPTY TREE]\033[0m\n");
	else
